...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cellular Automata Studio v2.0")
    ...
    parser.add_argument("--theme", type=str, choices=["light", "dark", "matrix", "ocean"], default="dark")
    ...
    parser.add_argument("--speed", type=int, default=50, help="Simulation speed in ms")
...
def check_dependencies() -> dict:
    status = {'numpy': False, 'pyside6': False, 'numba': HAS_NUMBA, 'cupy': HAS_CUPY, 'scipy': False}
    try:
...
def main() -> None:
    ...
    if args.cell_size != 5: ca.set_cell_size(args.cell_size)
    if args.rule != "B3/S23": 
        ca.set_rule(args.rule)
        window.rule_combo.setEditText(args.rule)
    if args.max_state != 16: ca.set_max_state(args.max_state)
    if args.fast: 
